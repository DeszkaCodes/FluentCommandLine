...
public partial interface ICommandBaseCanSetProperties : IIdentifierSymbolCanSetProperties
{
    ...
    ICommandBaseCanSetProperties SetHandler<T>(Func<T, Task> handle, IValueDescriptor<T> symbol);

    /// <returns>An <see cref="ICommandBaseCanSetProperties"/> with the new handler.</returns>
    /// <inheritdoc cref="Handler.SetHandler{T1, T2}(Command, Func{T1, T2, Task}, IValueDescriptor{T1}, IValueDescriptor{T2})"/>
    ICommandBaseCanSetProperties SetHandler<T1, T2>(Func<T1, T2, Task> handle,
